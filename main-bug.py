...
class Car:
	...
	def update(self, road):
		...
		self.pos[1] += math.cos(math.radians(360 - self.angle)) * self.speed
		if self.pos[1] < 20:
			self.pos[1] = 20
		elif self.pos[1] > height - 120:
			self.pos[1] = height - 120
            
		#upade distance & time spent
...
def run_generation(genomes, config):
    ...
    road = pygame.image.load("roadImg/road-4.png")
    
    font = pygame.font.SysFont("Press Start 2P", 36)
    heading_font = pygame.font.SysFont("Press Start 2P", 60)
    ...
    while True:
        ...
        label = heading_font.render(u"Поколение: " + str(generation), True, (110,197,38))
        label_rect = label.get_rect()
        label_rect.center = (width / 4, 40)
        screen.blit(label, label_rect)
        
        label = font.render(u"Машин осталось: " + str(cars_left), True, (51,59,70))
        label_rect = label.get_rect()
        label_rect.center = (width / 4, 80)
        screen.blit(label, label_rect)
